package cn.jxau.bookstore.user.web.servlet;
...
import cn.jxau.bookstore.user.service.UserException;
import cn.jxau.bookstore.user.service.UserService;
...
public class UserServlet extends BaseServlet {
	...
	public String login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/*
		 * 1. 封装表单数据到form中 2. 输入校验 3. 调用service完成激活 >
		...
		try {
			User user = userService.load(form);
	...
	public String regist(HttpServletRequest request, HttpServletResponse response)
		...
		if (username == null || username.trim().isEmpty()) {
			...
			errors.put("username", "用户名长度必须在在3~10之间");
		}
		String password = form.getPassword();
		if (password == null || password.trim().isEmpty()) {
			errors.put("password", "密码不能为空");
		} else if (password.length() < 3 || password.length() > 10) {
			errors.put("password", "密码长度必须在在3~10之间");
		}
		String email = form.getEmail();
		String regular = "\\w+@\\w+\\.\\w+";
		if (email == null || email.trim().isEmpty()) {
			errors.put("email", "email不能为空");
		} else if (!email.matches(regular)) {
		 ...
		 */
		request.setAttribute("msg", "恭喜，已成功激活，请前往邮箱激活");
		return "f:/jsps/msg.jsp";
